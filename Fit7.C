...
TCanvas* c = new TCanvas("c","test",570,400);
  ...
  RooDataHist ds("ds","ds",mW,Import(*hist7)) ;
  ...
  */
  ...
  // pull 
  RooPlot* z1frame = mW.frame(Title("Wmass in 180 < Pt <210"));
  RooHist* hpull1 = frame->pullHist();
  z1frame->addPlotable(hpull1,"P");

  RooPlot* z2frame = mW.frame(Title("Wmass in 180 < Pt <210"));
