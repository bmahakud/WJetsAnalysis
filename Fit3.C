...
TCanvas* c = new TCanvas("c","test",570,400);
  ...
  RooDataHist ds("ds","ds",mW,Import(*hist3)) ;
  ...
  RooRealVar sigma("sigma","sigma",8,5,30.0);
  ...
  */
  ...
  // pull 
  RooPlot* z1frame = mW.frame(Title("Wmass in 60 < Pt <90"));
  RooHist* hpull1 = frame->pullHist();
  z1frame->addPlotable(hpull1,"P");

  RooPlot* z2frame = mW.frame(Title("Wmass in 60 < Pt <90"));
